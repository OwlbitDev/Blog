	...
	>
		...
		<div class="hidden md:flex items-center space-x-2">
			{#each navItems as item}
				<a
					...
					aria-label={locale.nav[item]}
				>
					{locale.nav[item]}
					<span
						class="absolute -bottom-1 left-1/2 w-0 h-0.5 bg-primary dark:bg-dark-primary transition-all duration-300 group-hover:w-4/5 group-hover:left-1/10"
					></span>
				</a>
			{/each}
			...
			<!-- 工具组件容器 -->
			<div
				class="flex items-center space-x-2 ml-4 pl-4 border-l border-outline-variant dark:border-dark-outline-variant"
			>
				<ThemeMode />
				<I18n {lang} />
			</div>
		...
		<div class="flex md:hidden items-center space-x-3">
			...
			<button
				...
				aria-label={mobileMenuOpen ? locale.nav.closemenu : locale.nav.openmenu}
