...
<div class="projects-page min-h-screen bg-surface dark:bg-dark-surface">
	<!-- 页面头部 -->
	<section
		class="page-header dark:page-header-dd bg-surface-container dark:bg-dark-surface-container py-16 px-6"
	>
			...
			<div class="filters-container mb-12">
				<div class="flex flex-wrap gap-2 justify-center">
					{#each filters as filter}
						<button
							class="filter-btn px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 {activeFilter ===
							filter.id
			...
			<div class="projects-grid">
				...
				{#if getFeaturedProjects().length > 0}
					<div class="featured-section mb-12">
						<h2
							class="text-2xl font-semibold text-on-surface dark:text-dark-on-surface mb-6 flex items-center"
						>
							<svg
								class="w-6 h-6 mr-2 text-primary dark:text-dark-primary"
								fill="currentColor"
								viewBox="0 0 20 20"
							>
								<path
				...
				{#if getFilteredProjects().length === 0}
					<div class="empty-state text-center py-16">
						<svg
							class="w-24 h-24 mx-auto mb-6 text-on-surface-variant dark:text-dark-on-surface-variant opacity-50"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
...
<style>
	...
	.page-header-dd {
		background: linear-gradient(
			135deg,
			var(--color-dark-surface-container) 0%,
			var(--color-dark-surface-container-low) 100%
		);
	}
	/* 平滑滚动 */
	html {
